...
// ⚠️ Metadata: move to app/pricing/layout.tsx (can't export from "use client"):
  ...
  return (
    <div className="flex items-center justify-center gap-3 mt-7 select-none">
      <span
        ...
        className={`font-mono text-[13px] cursor-pointer transition-colors duration-200 ${
          !isAnnual ? "text-gray-900 font-bold" : "text-gray-400"
        }`}
      >
        Monthly
      </span>
      ...
      <span
        onClick={() => !isAnnual && onToggle()}
        className={`font-mono text-[13px] cursor-pointer flex items-center gap-1.5 transition-colors duration-200 ${
          isAnnual ? "text-gray-900 font-bold" : "text-gray-400"
        }`}
      >
        Annual
  ...
  return (
    <div className="mb-2">
      <div className="flex items-end gap-1 mb-0.5">
        {/* Fixed-width container prevents layout jump on price change */}
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <p className="px-5 py-4 font-mono text-[13px] text-gray-600 leading-relaxed">
              {item.a}
            </p>
          </motion.div>
        )}
...
export default function PricingPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // ── Table rows: stagger fade+slide on scroll ──
      ...
      // ── Highlighted card: subtle glow pulse (GSAP loop) ──
      ...
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-6">
        <div className="max-w-[640px]">
          ...
          <motion.div custom={2} variants={heroVariants} initial="hidden" animate="visible">
            <TrustBar
              items={["No setup fees", "Cancel anytime", "SOC 2 ready", "HIPAA compliant", "14-day free trial"]}
            />
          </motion.div>
        </div>
      ...
      <section className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 pb-16 pt-8">
        ...
        >
          {PLANS.map((plan) => (
            ...
            >
              {/* GSAP glow target (absolute overlay, pointer-events-none) */}
              {plan.highlight && (
                <div
                  ref={glowLayerRef}
      ...
      >
        ...
        <div className="overflow-x-auto rounded-2xl border border-gray-100 mt-8">
          <table className="w-full text-[13px] font-mono min-w-[640px]">
            <thead className="sticky top-0 z-10">
              <tr className="border-b border-gray-100 bg-gray-50/95 backdrop-blur-sm">
                ...
                {PLANS.map((p) => (
                  <th
                    key={p.name}
                    className={`text-center px-5 py-4 text-[12px] font-bold ${
                      p.highlight ? "text-accent" : "text-gray-800"
                    }`}
                  >
                    {p.name}
                    <AnimatePresence mode="wait" initial={false}>
            ...
            {/* GSAP animates these rows on scroll */}
                  ...
                  {PLANS.map((p) => (
                    <td
                      ...
                      className={`px-5 py-3.5 text-center ${
                        p.highlight ? "hover:bg-blue-50/25 transition-colors" : ""
                      }`}
                    >
                      {p.features[feature] ? (
                        <motion.span
